...
onMounted(async()=>{
    if (articleId) {
        ...
        article.value.id = res.data.data.id
        article.value.title = res.data.data.title
        article.value.summary = res.data.data.summary
        article.value.body = res.data.data.body,
        article.value.status = res.data.data.status
        imgUrl.value = res.data.data.img

        // 重新构建 HTML 内容
        valueHtml.value = `<p style="text-align: center;">${article.value.title}</p>
            <p style="text-indent: 2em;">${article.value.summary}</p>
            ${article.value.body}
        `
    }
})
